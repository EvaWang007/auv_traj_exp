...
class RNNCurrentPredictor(nn.Module):
...
def save_plots(out_dir, label, sample_idx, X_raw, true_pos, pred_pos, true_cur, pred_cur):
    ...
    fig_path = out_dir / f'fig_prediction_rnn_true_current_integral_{label}.png'
    ...
    plt.plot(pred_pos[sample_idx, :, 0], pred_pos[sample_idx, :, 1], '--', linewidth=2, label='RNN current-integral')
    plt.axis('equal')
    plt.grid(True)
    plt.legend()
    plt.title(f'RNN true-current integral trajectory: {label}')
    plt.tight_layout()
    plt.savefig(fig_path, dpi=200)
    plt.close()
    print(f'[OK] saved {fig_path}')

    cur_fig_path = out_dir / f'fig_current_rnn_true_current_integral_{label}.png'
    ...
    plt.legend()
    plt.title(f'RNN true-current prediction: {label}')
    plt.tight_layout()
    plt.savefig(cur_fig_path, dpi=200)
    plt.close()
    print(f'[OK] saved {cur_fig_path}')

    sample_csv = out_dir / f'current_prediction_sample_rnn_true_current_integral_{label}.csv'
    pd.DataFrame({
        'step': k,
...
def run(args):
    ...
    model = RNNCurrentPredictor(input_dim=5, hidden_dim=args.hidden_dim, pred_len=args.pred_len).to(device)
    ...
    for ep in range(1, args.epochs + 1):
        ...
        if ep == 1 or ep % args.log_every == 0 or ep == args.epochs:
            print(
                f'[RNN_TRUE_CURRENT] epoch={ep:03d} '
    ...
    result = {
        'model': 'RNN_TrueCurrent_Integral',
    ...
    result_csv = out_dir / f'results_rnn_true_current_integral_{args.dataset_label}.csv'
    history_csv = out_dir / f'train_history_rnn_true_current_integral_{args.dataset_label}.csv'
    model_path = out_dir / f'rnn_true_current_integral_{args.dataset_label}.pt'
