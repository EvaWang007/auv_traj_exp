...
def load_src():
    spec = importlib.util.spec_from_file_location("auv_smoke_pso", SRC)
...
def decode_position(position: np.ndarray, space: SearchSpace):
    ...
    idxs = []
    params = {}
    names = ["hidden_dim", "num_layers", "dropout", "lr", "batch_size"]
...
def train_model(module, model, train_loader, val_loader, y_scaler, dt, epochs, lr, device, name):
    model.to(device)
...
def evaluate_candidate(
    ...
    train_loader, val_loader, _, y_scaler = build_loaders(
        module,
        X_train_s,
        Y_train_s,
        X_val_s,
    ...
    model = train_model(
        ...
        lr=params["lr"],
        device=device,
        name=f"PSO-candidate-{params}",
    )
    pred_val, true_val = predict_model(module, model, val_loader, y_scaler, device)
    metrics = module.metrics_np(pred_val, true_val)
    fitness = metrics["ADE"] + 0.5 * metrics["FDE"]
    return fitness


def run_pso(module, args, out_dir: Path):
    ...
    # Search uses fixed window subsets from the same train/val split for tractability.
    ...
    for it in range(1, args.iterations + 1):
        ...
        best_key, best_params = decode_position(gbest_pos, space)
        print(f"[PSO] gbest_rmse={gbest_fit:.6f} best_params={best_params}")

    history_df = pd.DataFrame(history_rows)
    history_path = out_dir / f"pso_search_history_{args.dataset_label}.csv"
    history_df.to_csv(history_path, index=False)

    _, best_params = decode_position(gbest_pos, space)
    best_params["fitness_ade_fde"] = float(gbest_fit)
    best_path = out_dir / f"best_hparams_pso_lstm_adefde_{args.dataset_label}.json"
    ...
    # Final retraining on the same train/val/test split for fair comparison to prior LSTM runs.
    train_loader, val_loader, _, y_scaler = build_loaders(
        module,
        X_train,
        Y_train,
    ...
    results_df = pd.DataFrame([{"model": "pso_lstm_adefde", **metrics, **best_params}])
    results_path = out_dir / f"results_pso_lstm_adefde_{args.dataset_label}.csv"
    results_df.to_csv(results_path, index=False)
    print(results_df)
    print(f"[OK] saved {results_path}")

    model_path = out_dir / f"pso_lstm_adefde_{args.dataset_label}.pt"
    torch.save(final_model.state_dict(), model_path)
    print(f"[OK] saved {model_path}")

    # Plot convergence and one prediction sample.
    gbest_curve = history_df.groupby("iteration")["fitness_ade_fde"].min().cummin().reset_index()
    plt.figure(figsize=(6, 4))
    plt.plot(gbest_curve["iteration"], gbest_curve["fitness_ade_fde"], marker="o")
    plt.grid(True)
    plt.xlabel("Iteration")
    plt.ylabel("Best validation RMSE")
    plt.title(f"PSO search convergence: {args.dataset_label}")
    conv_path = out_dir / f"fig_pso_convergence_{args.dataset_label}.png"
    plt.tight_layout()
    ...
    pso_pred = pred_test[sample_idx]
    plt.plot(hist[:, 0], hist[:, 1], "ko-", label="input history(nav)", linewidth=1)
    plt.plot(gt[:, 0], gt[:, 1], "g-", label="ground truth", linewidth=2)
    plt.plot(pso_pred[:, 0], pso_pred[:, 1], "--", label="PSO-LSTM-ADEFDE")
    plt.axis("equal")
    plt.grid(True)
    plt.legend()
    plt.title(f"AUV PSO-LSTM-ADEFDE prediction sample: {args.dataset_label}")
    pred_fig_path = out_dir / f"fig_prediction_pso_lstm_adefde_{args.dataset_label}.png"
    plt.tight_layout()
...
def main():
    ...
    run_pso(module, args, Path(args.out_dir))
