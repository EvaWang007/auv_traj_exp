...
def final_train(pi_module, base_module, params, X_train, Yp_train, Yc_train, Yb_train, X_val, Yp_val, Yc_val, Yb_val, args, device):
    ...
    for ep in range(1, args.final_epochs + 1):
        ...
        if ep == 1 or ep % args.log_every == 0 or ep == args.final_epochs:
            print(
                f"[PSO_BCPI_LSTM_final] epoch={ep:03d} train_total={train_losses[0]:.6f} "
...
def run(args):
    ...
    for it in range(1, args.iterations + 1):
        print(f'\n=== PSO-BCPI iteration {it}/{args.iterations} ===')
        for p in range(args.population):
            ...
            row = {'iteration': it, 'particle': p, 'fitness_val_total': fitness, 'from_cache': from_cache, **params}
        ...
        print(f'[PSO-BCPI] best_fitness={gbest_fit:.6f} best_params={best_params}')

    _, best_params = decode_position(gbest_pos, space)
    history_csv = out_dir / f'pso_search_history_bcpi_lstm_{args.dataset_label}.csv'
    best_json = out_dir / f'best_hparams_pso_bcpi_lstm_{args.dataset_label}.json'
    pd.DataFrame(history_rows).to_csv(history_csv, index=False)
    with open(best_json, 'w', encoding='utf-8') as f:
        json.dump({'best_params': best_params, 'best_fitness_val_total': gbest_fit}, f, indent=2)
    ...
    result = {
        ...
        **best_params,
        'fitness_val_total': gbest_fit,
        'final_epochs': args.final_epochs,
    }
    result_df = pd.DataFrame([result])
    print(result_df)

    result_csv = out_dir / f'results_pso_bcpi_lstm_{args.dataset_label}.csv'
    final_history_csv = out_dir / f'train_history_pso_bcpi_lstm_{args.dataset_label}.csv'
    model_path = out_dir / f'pso_bcpi_lstm_{args.dataset_label}.pt'
    ...
    pi_module.save_plots(
        out_dir, args.dataset_label, 'pso_bcpi_lstm', args.sample_idx,
    ...
    best_curve = hist_df.groupby('iteration')['fitness_val_total'].min().cummin()
    plt.figure(figsize=(7, 4))
    plt.plot(best_curve.index, best_curve.values, marker='o')
    plt.grid(True)
    plt.xlabel('PSO iteration')
    plt.ylabel('best validation total loss')
    plt.title(f'PSO-BCPI-LSTM convergence: {args.dataset_label}')
    plt.tight_layout()
