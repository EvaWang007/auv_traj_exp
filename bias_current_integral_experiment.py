...
def load_src():
    spec = importlib.util.spec_from_file_location('auv_smoke_bias_current', SRC)
...
def run_epoch(model, loader, opt, cur_scaler, bias_scaler, dt, pos_scale, current_scale,
              alpha_cur, alpha_bias, beta_smooth, device):
    model.train()
    totals = np.zeros(5, dtype=np.float64)
    for batch in loader:
        xb, ycur, ybias, xraw, ypos, _, _ = unpack_batch(batch, device)
        ...
        loss = loss_pos + alpha_cur * loss_cur + alpha_bias * loss_bias + beta_smooth * loss_smooth
        ...
        n = len(xb)
        totals += np.array([loss.item(), loss_pos.item(), loss_cur.item(), loss_bias.item(), loss_smooth.item()]) * n
...
def run(args):
    ...
    model_tag = f'{args.cell}_bias_current_integral'
    print(
    ...
    print(
        ...
        f'{args.alpha_bias} * bias_loss + {args.beta_smooth} * smoothness_loss'
    )

    model = BiasCurrentPredictor(input_dim=5, hidden_dim=args.hidden_dim, pred_len=args.pred_len, cell=args.cell).to(device)
    ...
    for ep in range(1, args.epochs + 1):
        train_losses = run_epoch(
            ...
            args.alpha_cur, args.alpha_bias, args.beta_smooth, device,
        )
        ...
        if ep == 1 or ep % args.log_every == 0 or ep == args.epochs:
            print(
                ...
                f'val_current={val_losses[2]:.6f} val_bias={val_losses[3]:.6f}'
            )
...
def main():
    ...
    parser.add_argument('--cell', choices=['rnn', 'lstm'], required=True)
