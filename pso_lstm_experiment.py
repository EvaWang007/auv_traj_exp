...
def evaluate_candidate(
    ...
    fitness = module.metrics_np(pred_val, true_val)["RMSE"]
    return fitness
...
def run_pso(module, args, out_dir: Path):
    ...
    best_path = out_dir / f"best_hparams_pso_lstm_{args.dataset_label}.json"
    ...
    results_df = pd.DataFrame([{"model": "PSO_LSTM", **metrics, **best_params}])
    results_path = out_dir / f"results_pso_lstm_{args.dataset_label}.csv"
    results_df.to_csv(results_path, index=False)
    print(results_df)
    print(f"[OK] saved {results_path}")

    model_path = out_dir / f"pso_lstm_{args.dataset_label}.pt"
    ...
    plt.plot(pso_pred[:, 0], pso_pred[:, 1], "--", label="PSO-LSTM")
    plt.axis("equal")
    plt.grid(True)
    plt.legend()
    plt.title(f"AUV PSO-LSTM prediction sample: {args.dataset_label}")
    pred_fig_path = out_dir / f"fig_prediction_pso_lstm_{args.dataset_label}.png"
    plt.tight_layout()
