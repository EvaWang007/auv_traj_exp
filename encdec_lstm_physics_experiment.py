...
def load_src():
    spec = importlib.util.spec_from_file_location("auv_smoke_encdec_phy", SRC)
...
def train_encdec_model(
    ...
    for ep in range(1, epochs + 1):
        ...
        val_loss = evaluate_encdec_loss(module, model, val_loader, y_scaler, dt, lambda_phy, teacher_forcing_ratio, device)
...
@torch.no_grad()
def evaluate_encdec_loss(module, model: nn.Module, loader: DataLoader, y_scaler, dt: float, lambda_phy: float, teacher_forcing_ratio: float, device: str) -> float:
    model.eval()
    total = 0.0
    position_scale = float(np.mean(y_scaler.std))
...
def run_experiment(module, npz_path: Path, out_dir: Path, label: str, args):
    print(f"\n=== Running PI-EncDec-LSTM comparison dataset: {label} ===")
    ...
    pi_lstm = module.RNNPredictor(input_dim=5, hidden_dim=args.hidden_dim, pred_len=args.pred_len, cell="lstm")
    pi_lstm = module.train_model(
        "PI_LSTM",
        pi_lstm,
        train_loader,
        val_loader,
    ...
    pred_pi_lstm, true_pi_lstm, _ = module.predict_model(pi_lstm, test_loader, y_scaler, device)
    results.append({"model": f"PI_LSTM_lambda_{args.lambda_phy}", **module.metrics_np(pred_pi_lstm, true_pi_lstm)})
    ...
    pred_pi_encdec, true_pi_encdec, _ = predict_encdec_model(pi_encdec, test_loader, y_scaler, device)
    results.append({"model": f"PI_EncDec_LSTM_lambda_{args.lambda_phy}", **module.metrics_np(pred_pi_encdec, true_pi_encdec)})

    df = pd.DataFrame(results)
    out_csv = out_dir / f"results_encdec_physics_{label}.csv"
    df.to_csv(out_csv, index=False)
    print(df)
    print(f"[OK] saved {out_csv}")

    torch.save(lstm.state_dict(), out_dir / f"lstm_oneshot_{label}.pt")
    torch.save(pi_lstm.state_dict(), out_dir / f"pi_lstm_oneshot_{label}.pt")
    torch.save(encdec.state_dict(), out_dir / f"encdec_lstm_{label}.pt")
    torch.save(pi_encdec.state_dict(), out_dir / f"pi_encdec_lstm_{label}.pt")
    ...
    plt.plot(pred_pi_lstm[sample_idx, :, 0], pred_pi_lstm[sample_idx, :, 1], "--", label="PI-LSTM")
    ...
    plt.grid(True)
    plt.legend()
    plt.title(f"AUV PI-EncDec-LSTM comparison: {label}")
    fig_path = out_dir / f"fig_prediction_encdec_physics_{label}.png"
    plt.tight_layout()
